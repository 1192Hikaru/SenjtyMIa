...
var undevelopedDangerZones = [
    {lat: 35.747936, lng: 139.806729, name: "交通事故警戒地帯1demo"},
    {lat: 35.748833, lng: 139.807534, name: "交通事故警戒地帯2demo"},
    {lat: 35.9602, lng: 139.7891, name: "交通事故警戒地帯"}
];
...
var agingMarkers = [
    ...
    },
    {
        lat: 35.748715,
        lng: 139.805930,
        name: "道路舗装demo",routeName: "東京都足立区千住旭町5",
        inspectionYear: 2022,
        score: "Ⅱ「demo」",
...
var undevelopedDangerLayer = L.layerGroup();
...
undevelopedDangerZones.forEach(function(zone) {
    var marker = L.marker([zone.lat, zone.lng], { icon: redIcon2 }).addTo(undevelopedDangerLayer);
    marker.bindPopup(`<b>${zone.name}</b>`);
});

// 初期表示するレイヤーを設定
var overlayMaps = {
    "交通事故多発地帯": dangerLayer,
...
undevelopedDangerLayer.addTo(map);
...
function checkDangerZones(userLatLng) {
    ...
    activeLayers.forEach(function(layerGroup) {
        layerGroup.eachLayer(function(layer) {
            ...
            if (distance < 100 && !alertTriggered && (currentTime - lastAlertTimestamp) > 3000) { // 距離が10m以内かつ　3秒間隔でアラートを一度だけ鳴らす 50m
